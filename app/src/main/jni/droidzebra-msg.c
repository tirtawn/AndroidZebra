...
droidzebra_msg_board(
    ...
    buffer_pos += sprintf(buffer + buffer_pos, "\"disks_played\":%d,",
                          side_to_move == BLACKSQ ? 2 * score_sheet_row : 2 * score_sheet_row + 1);
    ...
    // if it is white move then current score_sheet_row has the black move so we need to look at it as well
    len = (side_to_move == BLACKSQ ? score_sheet_row : score_sheet_row + 1);
    for (i = 0; i < len; i++)
...
droidzebra_msg_candidate_moves(void) {
    ...
    for (i = 0; i < move_count[disks_played]; i++) {
        buffer_pos += sprintf(buffer + buffer_pos, "{\"move\":%d},", move_list[disks_played][i]);
    }
